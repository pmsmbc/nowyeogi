...
- **굵은 글씨(`**...**`) 뒤에 한글이 바로 붙으면 마크다운이 닫지 못한다.** `**"..."**예요` 처럼 쓰지 말고 `**...**는 거예요` 처럼 따옴표를 밖으로 빼거나 조사 앞에 공백을 둔다.
- **여는 `**` 바로 뒤에 따옴표·괄호가 와도 깨진다.** 닫는 쪽만 조심하면 되는 게 아니다. `とくに**「…」と` 나 `시가**"…"를` 처럼 **여는 별표 앞이 글자이고 뒤가 구두점**이면 마크다운이 열지 못한다. 괄호·따옴표를 굵은 글씨 밖으로 빼거나(`**…**` 안에 따옴표를 넣지 않는다), 여는 별표 앞에 공백이나 쉼표를 둔다.
- **일본어 글도 같은 규칙이 적용된다.** `**텍스트。**다음` 처럼 닫는 `**` 앞에 `。、）」』` 가 오고 뒤에 글자가 붙으면 마크다운이 닫지 못한다. `**텍스트**。다음` 으로 구두점을 밖에 두고, 괄호는 `**이름**（요미）` 처럼 통째로 밖에 둔다. `****` 처럼 별표가 이어지지 않게 한다.
- **배지 토큰을 굵은 글씨로 감싸지 않는다.** `**지하철 [[line:busan2]]**이` 처럼 닫는 별표 앞이 `]]` 면 구두점으로 취급돼 굵은 글씨가 닫히지 않는다. 배지는 굵게 할 필요가 없으니 `지하철 [[line:busan2]] 가` 처럼 그냥 둔다.
- **지하철 노선은 반드시 `[[line:4]]`, `[[line:gyeongui]]`, `[[line:arex]]` 처럼 쓴다.** 노선 색 배지로 자동 표시되고 언어에 맞는 이름이 붙는다. 키 목록은 `src/data/subway.json`.
- **버스 번호도 `[[bus:6001]]`, `[[bus:400]]` 처럼 쓴다.** 번호로 종류(공항·간선·지선·광역·순환)를 자동 판별해 색을 넣고, 한국어는 "6001번", 영어는 "Bus 6001" 로 표시된다. 판별이 틀리면 `[[bus:green:6002]]` 처럼 종류를 직접 지정한다. 종류 키는 `src/data/bus.json`.
- 배지는 본문에만 쓴다. 제목(H1/H2)이나 frontmatter 에는 쓰지 않는다. 없는 노선·종류는 데이터 파일에 먼저 추가한다.
