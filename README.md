...
- **사진은 자유 라이선스만.** 직접 찍은 사진, 또는 `npm run fetch-images` 로 받은 위키미디어 공용 사진(CC0·CC BY·CC BY-SA)만 쓴다. 출처는 사진 아래에 자동으로 표시된다.
...
| `npm test` | 스크립트 테스트 |
| `npm run fetch-images -- <slug> "<영어 검색어>"` | 위키미디어 공용에서 자유 라이선스 사진 받기(출처 자동 표기) |
